...
no *adicionar(no *raiz, registro pessoa){
    if(raiz == NULL){
        ...
        printf("%s e %s\n", raiz->dados.nome, pessoa.pai);
        system("pause");
        if(raiz->dados.nome == pessoa.pai){
            if(raiz->esq == NULL)
            ...
            if(raiz->esq != NULL && raiz->dados.nome != pessoa.pai)
                raiz->esq = adicionar(raiz->esq, pessoa);
            if(raiz->dir != NULL && raiz->dados.nome != pessoa.pai)
                raiz->dir = adicionar(raiz->dir, pessoa);
        }
    }
    return raiz;
...
void filhos(no *raiz, char pai[tam]){
    if(raiz != NULL){
        if(raiz->dados.nome == pai)
            printf(" %s e %s", raiz->esq->dados.nome, raiz->dir->dados.nome);
        else {
            filhos(raiz->esq, pai);
            filhos(raiz->dir, pai);
        }
...
int quintaGeracao(no *raiz, char pai[tam], int cont){
    if(raiz != NULL){
        if(cont == 5)
            printf(" %s.", raiz->dados.nome);

        cont = quintaGeracao(raiz->esq, pai, ++cont);
        cont = quintaGeracao(raiz->dir, pai, ++cont);
    }
    return --cont;
};
...
int bisneto(no *raiz, char pai[tam], int cont){
    if(raiz != NULL){
        if(cont == 4)
            printf(" %s.", raiz->dados.nome);

        cont = bisneto(raiz->esq, pai, ++cont);
        cont = bisneto(raiz->dir, pai, ++cont);
    }
    return --cont;
};
